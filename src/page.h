...
struct tx_details
{
    ...
    get_mstch_map() const
    {
        ...
        if (!input_key_imgs.empty())
        {
            ...
            fee_short_str    = fmt::format("{:0.8f}", xmr_amount);
            payed_for_kB_str = fmt::format("{:0.8f}", payed_for_kB);
        }
        ...
        mstch::map txd_map {
                ...
                {"sum_inputs_short"  , xmr_amount_to_str(xmr_inputs , "{:0.8f}")},
                {"sum_outputs_short" , xmr_amount_to_str(xmr_outputs, "{:0.8f}")},
                ...
                {"tx_size_short"     , fmt::format("{:0.8f}", tx_size)},
...
public:
    ...
    index2(uint64_t page_no = 0, bool refresh_page = false)
    {
        ...
        while (i >= start_height)
        {
            ...
            string blk_size_str = fmt::format("{:0.8f}", blk_size);
        ...
        if (unsigned_tx_given)
        {
            ...
            if (r)
            {
                ...
                for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
                {
                    ...
                    tx_cd_data["timescales_scale"] = fmt::format("{:0.8f}",
...
private:
    ...
    construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
    {
        ...
        if (detailed_view)
        {
            ...
            context["timescales_scale"] = fmt::format("{:0.8f}",
